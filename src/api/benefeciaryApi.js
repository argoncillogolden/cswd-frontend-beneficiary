...
export const BENEFECIARY_API = {
    ...
    // Update
    async updateReleasingCashStatusApi(formData) {
        try {
            const authToken = localStorage.getItem("auth_token");
            if (!authToken) {
                throw new Error("No authentication token found");
            }
            const hand_over_event_id = formData.get("hand_over_event_id");
            if (!hand_over_event_id) {
                throw new Error("Hand-over event ID is required for update");
            }
            const config = {
                headers: {
                    Authorization: `Bearer ${authToken}`,
